...
def f_to_istr(width, f):
    ...
    maxno = pow(2, width-2)
    return str(int(f *(2**16)))
    #return str(hex(round((f*65536)))
...
#make_twiddle_factor_file(N=16,tf_width=32,output_fn="rom.v")

make_twiddle_factor_file(N=32,tf_width=32,output_fn="rom.v",template_fn="rom_N.v.t")
